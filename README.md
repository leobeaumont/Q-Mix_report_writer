...
- Contextual chunk prefixing: each chunk is embedded with a one-line prefix `[Source: filename | Page: N]` prepended to the text. The stored document text remains the original (no prefix). This anchors semantically vague chunks to their document and page context at embedding time. A per-text embedding ceiling of 2900 characters (`_EMBED_MAX_CHARS`) guards against an Ollama server-side character limit.
