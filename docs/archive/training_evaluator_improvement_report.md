...
> **Status: EXECUTED (2026-07-09) — plan complete, archived.** All 34 tracker items done (Stages 0–5 + the live-smoke follow-ups). Headline results: scorer ranking accuracy **0.35 → 0.85** with a fully deterministic reward (repeat σ 0.0127 → 0.000) and TD3 grounding live (claim check + client-side evidence verification); trainer fixed (masked Double-DQN, terminal transitions, vectorized batched forward, in-edge GNN aggregation); training loop rebuilt (replay ratio + warmup, greedy eval + best checkpoint, JSONL reward decomposition, seeding, checkpoint v2 + buffer persistence + real resume). Verified end-to-end live (2-episode smoke, 2026-07-09: grounded rewards + terminal macro on real reports). The *what/when/done* record is [`training_eval_upgrade_plan.md`](training_eval_upgrade_plan.md) (archived alongside this report).
